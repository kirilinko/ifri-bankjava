...
<html>
    ...
    <head>
		<title>Liste des Cients par agence</title>
	 ...
	 <div class="container text-end mb-2 ">
      ...
      <div class="collapse navbar-collapse" id="navbarText">
         ...
         <li class="nav-item active">
           <a class="nav-link font-weight-bold text-white" href="/client/agence"><i class="fa fa-building" aria-hidden="true"></i> Client par Agence</a>
         </li>
         <li class="nav-item">
  ...
  <div class="container">
       ...
       <h4 class="text mb-4">Liste des clients par agence</h4>
       
        <c:url var="search_client_url" value="/client/agence"/>
       <form:form action="${search_client_url}" method="get" class="form-inline">
         <div class="row">
           
             <div class="col-sm mx-sm-3 mb-2">
                 <label class="sr-only">Agence</label>
                 <select name="agenceid" class="form-control">   
                    <option value="0">Choisir une agence</options>
                 <c:forEach  items="${agences}" var="agence">
					<option value="${agence.numeroagence}">${agence.bank.nombanque} ${agence.ville}</option>
				  </c:forEach>  
                </select>
             </div>
           ...
           <tbody>
            <c:forEach items="${clients}" var="client">
              <tr>
                <th>${client.nom}</th>
                <td>${client.prenom}</td>
                <td>${client.agence.bank.nombanque} - ${client.agence.ville}</td>
                <td>${client.agence.adresseag}</td> 
              </tr>
            </c:forEach> 
          </tbody>
      </table>
